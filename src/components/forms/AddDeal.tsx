...
const AddDeal: React.FC<AddDealProps> = ({
  ...
  useEffect(() => {
    if (categories.length > 0) {
      setSelectedCategory(String(categories[0].value))
    }
  }, [categories])
  ...
  const onSubmit = async (data: FormType): Promise<void> => {
    try {
      ...
      handleError(error, 'Error creating Deal')
    } finally {
      setIsLoading(false)
    }
  }
        ...
        <View className='mb-4'>
          <TouchableOpacity>
            ...
            >
              <View style={{ flex: 1 }}>
                <Picker
                  selectedValue={selectedDealNature}
                  onValueChange={(itemValue) => {
                    setIsDealNatureProduct(
                      String(itemValue) === DEAL_NATURE_PRODUCT_ID
                    )
                    setSelectedDealNature(itemValue)
                  }}
                >
                  {dealNatures.map((c) => (
                    <Picker.Item
