/* eslint-disable max-len */
import { useRouter } from 'expo-router'
import Lottie from 'lottie-react-native'
import React from 'react'
import {
  Dimensions,
  StyleSheet,
  Text,
  TouchableOpacity,
  View,
} from 'react-native'
import Onboarding from 'react-native-onboarding-swiper'

import { setItem } from '../../lib/storage'
import { IS_FIRST_TIME_USER } from '../utils/constants'

const { width, height } = Dimensions.get('window')

const OnBoardingScreen = (): JSX.Element => {
  const router = useRouter()

  const handleDone = async (): Promise<void> => {
    await setItem<boolean>(IS_FIRST_TIME_USER, false)
    router.replace('/home')
  }

  const doneButton = ({ ...props }): JSX.Element => {
    return (
      <TouchableOpacity style={styles.doneButton} {...props}>
        <Text>Done</Text>
      </TouchableOpacity>
    )
  }
  return (
    <View style={styles.container}>
      <Onboarding
        onDone={handleDone}
        onSkip={handleDone}
        // bottomBarHighlight={false}
        DoneButtonComponent={doneButton}
        containerStyles={{ paddingHorizontal: 15 }}
        pages={[
          {
            backgroundColor: '#a78bfa',
            image: (
              <View style={styles.lottie}>
                <Lottie
                  source={require('../../assets/animations/delivery.json')}
                  autoPlay
                  loop
                />
              </View>
            ),
            title: 'Find Awesome Deals',
            subtitle:
              'By choosing from our accredited dealers, you can save your money and time',
          },
          {
            backgroundColor: '#a7f3d0',
            image: (
              <View style={styles.lottie}>
                <Lottie
                  source={require('../../assets/animations/boost.json')}
                  autoPlay
                  loop
                />
              </View>
            ),
            title: 'Get Top Services',
            subtitle: 'Find the perfect service provider for your needs',
          },
          {
            backgroundColor: '#fef3c7',
            image: (
              <View style={styles.lottie}>
                <Lottie
                  source={require('../../assets/animations/working.json')}
                  autoPlay
                  loop
                />
              </View>
            ),
            title: 'Market your Deals',
            subtitle:
              'Showcase and sell your deals seamlessly without interruption',
          },
        ]}
      />
    </View>
  )
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: 'white',
  },
  lottie: {
    width: width * 0.9,
    height: width,
  },
  doneButton: {
    padding: 20,
    // backgroundColor: 'white',
    // borderTopLeftRadius: '100%',
    // borderBottomLeftRadius: '100%'
  },
})

export default OnBoardingScreen
